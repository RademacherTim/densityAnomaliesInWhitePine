...
png (file = 'fig/WoodAgeVSDensityAnomalies.png', width =  1000, height = 400)
layout (matrix (1:2, nrow = 1, byrow = TRUE), widths = c (1, 1))
...
for (years in c ('all', 'select')) {
  ...
  #----------------------------------------------------------------------------------------
  temp <- data %>% filter (Year %in% Years) %>% 
  ...
  plot (x = tmp [['WoodAge']], y = tmp [['perc']], axes = FALSE, ylim = c (0, 100), 
        typ = 'l', col = 'white', xlab = 'Cambial wood age (years)', 
  ...
  #----------------------------------------------------------------------------------------
  if (years == 'select') {
    legend (x = 15, y = 40, box.lty = 0, col = c (colours, '#666666'), lwd = c (rep (2, 3), 3),
