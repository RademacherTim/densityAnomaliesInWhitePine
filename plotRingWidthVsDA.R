...
# Wrangle data to get frequency over ring width for breat height
#----------------------------------------------------------------------------------------
...
#----------------------------------------------------------------------------------------
par (mar = c (5, 5, 1, 1))
plot (rhoBH, las = 1, ylim = c (0, 0.25), xlab = 'Ring width (mm)', lwd = 3, axes = FALSE, 
...
legend (x = 7, y = 0.25, col = c (rep (colours [1], 2), '#666666'), lty = c (1, 2, 1), 
        legend = c ('with density anomaly','without density anomaly','all rings'),
        lwd = 3, box.lty = 0)
...
# Wrangle data to get frequency over ring width for near-branch
#----------------------------------------------------------------------------------------
...
tmp <-  rbind (tmp1, tmp2)
rhoBranch <- density (tmp [['RingWidth']])
rhoBranchwith <- density (tmp [['RingWidth']] [tmp [['DABH']] >= 1])
rhoBranchwithout <- density (tmp [['RingWidth']] [tmp [['DABH']] == 0])

# Plot graph of density distribution by ring width near a branch
#----------------------------------------------------------------------------------------
plot (rhoBranch, las = 1, ylim = c (0, 0.35), xlab = 'Ring width (mm)', lwd = 3, axes = FALSE,
      ylab = 'Density distribution of density anomalies', main = '', col = '#666666')
axis (1)
axis (2, las = 1)
lines (rhoBranchwith,    col = colours [2], lwd = 3, lty = 1)
lines (rhoBranchwithout, col = colours [2], lwd = 3, lty = 2)
legend (x = 7, y = 0.35, col = c (rep (colours [2], 2), '#666666'), lty = c (1, 2, 1), 
        legend = c ('with density anomaly','without density anomaly','all rings'),
        lwd = 3, box.lty = 0)
...
# Wrangle data to get frequency over ring width for top-of-tree
#----------------------------------------------------------------------------------------
...
tmp <-  rbind (tmp1, tmp2)
rho2010 <- density (tmp [['RingWidth']])
rho2010with <- density (tmp [['RingWidth']] [tmp [['DABH']] >= 1])
rho2010without <- density (tmp [['RingWidth']] [tmp [['DABH']] == 0])

# Plot graph of density distribution by ring width near a 2010
#----------------------------------------------------------------------------------------
plot (rho2010, las = 1, ylim = c (0, 0.35), xlab = 'Ring width (mm)', lwd = 3, axes = FALSE,
      ylab = 'Density distribution of density anomalies', main = '', col = '#666666')
axis (1)
axis (2, las = 1)
lines (rho2010with,  col = colours [3], lwd = 2, lty = 1)
lines (rhoBHwithout, col = colours [3], lwd = 2, lty = 2)
legend (x = 4.5, y = 0.35, col = c (rep (colours [3], 2), '#666666'), lty = c (1, 2, 1), 
        legend = c ('with density anomaly','without density anomaly','all rings'),
        lwd = 3, box.lty = 0)
