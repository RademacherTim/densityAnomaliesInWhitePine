...
# Wrangle data to get frequency over wood age for breat height, near-branch and top-of-tree for the high-frequency years only
#----------------------------------------------------------------------------------------
temp <- data %>% filter (Year %in%c (1999, 2002, 2012, 2016)) %>% 
  mutate (DABH     = ifelse (DABH_1 %in% 1:2 | DABH_2 %in% 1:2, 1, 
                             ifelse (is.na (DABH_1) | is.na (DABH_2), NA, 0)),
          DABranch = ifelse (DABranch_1 %in% 1:2 | DABranch_2 %in% 1:2, 1, 
                             ifelse (is.na (DABranch_1) | is.na (DABranch_2), NA, 0)),
...
# Plot frequency of occurence over age of the wood for the high-frequency years only
#----------------------------------------------------------------------------------------
par (mar = c (5, 5, 1, 1))
plot (x = tmp [['WoodAge']], y = tmp [['perc']], axes = FALSE, ylim = c (0, 100), 
      typ = 'l', col = 'white', xlab = 'Wood age (years)', 
...
# Add a legend 
#----------------------------------------------------------------------------------------
legend (x = 15, y = 50, box.lty = 0, col = c (colours, '#666666'), lwd = c (rep (2, 3), 3),
        legend = c ('breast height','near-branch','top-of-tree','all'))
#========================================================================================
